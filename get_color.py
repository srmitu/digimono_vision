...
class digimono_get_color(object):
    ...
    def found_std_num(self, median_num, array, minus_array):
    ...
    def color_detect(self, mode, status):
            ...
            for num in range(256):
                if(num < 180):
                    ...
                    if(self.h_array[num] <= 0):
                        self.minus_h[num] -= self.h_array[num]
                        self.h_array[num] = 0
                self.s_array[num] = old_s_array[num] - shared_s_array[num]
                self.v_array[num] = old_v_array[num] - shared_v_array[num]
                if(self.s_array[num] <= 0):
                    self.minus_s[num] -= self.s_array[num]
                    self.s_array[num] = 0
                if(self.v_array[num] <= 0):
                    self.minus_v[num] -= self.v_array[num]
                    self.v_array[num] = 0
        elif(mode == 0):
            if(status == True):
                self.add = 0
        ...
        threshold = []
        s_a_std_num1, s_a_std_num2 = self.found_std_num(s_a_med_num, s_a, self.minus_s) 
        v_a_std_num1, v_a_std_num2 = self.found_std_num(v_a_med_num, v_a, self.minus_v) 
        if(h_a_f2_med_num == 0):
            h_a_std_num1, h_a_std_num2 = self.found_std_num(h_a_med_num, h_a, self.minus_h) 
